...
from .configuration_quantum_retnet import QuantumRetNetConfig
...
class RetNetRelPos(nn.Module):
    def __init__(self, config: QuantumRetNetConfig):
        super().__init__()
        self.config = config
...
class MultiScaleRetention(nn.Module):
    def __init__(
        ...
        self.k_observables = ["XY" *(self.embed_dim//2)]
    ...
    def forward(
        ...
        if forward_impl == "parallel":
            ...
            retention_out, curr_kv = self.chunkwise_retention(Q, K, V, decay_mask)
        else:
...
class RetNetDecoderLayer(nn.Module):
    def __init__(self, config: QuantumRetNetConfig, depth: int, tensor_parallel: bool = False):
        super().__init__()
        self.config = config
        self.embed_dim = config.decoder_embed_dim
...
class RetNetPreTrainedModel(PreTrainedModel):
    ...
    config_class = QuantumRetNetConfig
